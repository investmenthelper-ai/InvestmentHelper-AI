...
def store_fivelets_in_neo4j(fivelets, document_text):
    ...
    text_embedding = getEmbedding(document_text)
    ...
    with driver.session() as session:
        ...
        for fivelet in fivelets:
            ...
            triplet_embedding = getEmbedding(triplet_text)
...
def parse_fivelet(fivelet_str):
    ...
    pattern = r'^\s*(?:\d+\.\s*)?\(\s*"(?P<Entity1>.*?)"\s*,\s*"(?P<Relation>.*?)"\s*,\s*"(?P<Entity2>.*?)"\s*,\s*(?P<RelatedCompanies>\[.*?\])\s*,\s*(?P<DateInterval>\[.*?\])\s*\)$'
...
def process_md_file(notification_text, companyName, notificationDate):
    """
    ...
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=1300, chunk_overlap=150, separators=["\n\n", "\n", ".", " ", ""])
...
def saveFileToKG(companyName, notification_text, notificationDate):
    fiveletsAndChunks = process_md_file(notification_text, companyName, notificationDate)
